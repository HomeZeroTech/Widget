...
Only these two files should be modified:
- `Acceptance/embed.js` - Main widget JavaScript
- `Acceptance/embed-styles.css` - Widget styles

**Do NOT edit:**
- Any files in `Production/` - deployed to live users
- `.min.js` or `.min.css` files - auto-generated by build
- `offline.html` - fallback page (rarely needs changes)
- Do not run the build command

## Architecture

Embeddable widget for HomeZero with two deployment environments:

- **Acceptance/** - Staging/test environment (EDIT FILES HERE)
- **Production/** - Live production environment (DO NOT EDIT)

Build: `terser` for JS minification, `lightningcss` for CSS minification.
